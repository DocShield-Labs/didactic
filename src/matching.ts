...
function getSimilarity(
  ...
  if (!isObject(expected) || !isObject(actual)) {
    const cmp = typeof comparators === 'function' ? comparators : exact;
    const result = cmp(expected, actual);
    return result.similarity ?? (result.passed ? 1.0 : 0.0);
  }

  // Objects: use exact for function comparators (they're meant for primitives)
