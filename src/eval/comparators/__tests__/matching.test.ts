...
describe('matchArrays', () => {
  describe('primitives', () => {
    it('matches identical arrays', async () => {
      const result = await matchArrays([1, 2, 3], [1, 2, 3]);
      expect(result.unmatchedExpected).toHaveLength(0);
      expect(result.assignments).toHaveLength(3);
    });

    it('matches reordered arrays', async () => {
      const result = await matchArrays([1, 2, 3], [3, 1, 2]);
      expect(result.unmatchedExpected).toHaveLength(0);
      expect(result.assignments).toHaveLength(3);
    });

    it('reports unmatched when expected items missing from actual', async () => {
      const result = await matchArrays([1, 2, 3], [1, 2]);
    ...
    it('allows extra actual items (lenient)', async () => {
      const result = await matchArrays([1, 2], [1, 2, 3]);
      expect(result.unmatchedExpected).toHaveLength(0);
      expect(result.unmatchedActual).toContain(2);
    });

    it('handles empty arrays', async () => {
      expect((await matchArrays([], [])).unmatchedExpected).toHaveLength(0);
      expect((await matchArrays([], [1])).unmatchedActual).toHaveLength(1);
      expect((await matchArrays([1], [])).unmatchedExpected).toHaveLength(1);
    });
  });
  ...
  describe('objects with exact matching', () => {
    ...
    it('matches identical object arrays', async () => {
      ...
      const result = await matchArrays(expected, actual);
      expect(result.unmatchedExpected).toHaveLength(0);
      expect(result.assignments).toHaveLength(2);
    });

    it('matches reordered object arrays', async () => {
      const expected: Quote[] = [
        { carrier: 'Acme', premium: 100 },
        { carrier: 'Beta', premium: 200 },
      ];
    ...
    it('matches objects even when some fields differ (similarity-based)', async () => {
    ...
    it('reports unmatched when expected object missing from actual', async () => {
      const expected: Quote[] = [
        { carrier: 'Acme', premium: 100 },
        { carrier: 'Beta', premium: 200 },
      ];
    ...
    it('allows extra objects in actual (lenient)', async () => {
      const expected: Quote[] = [{ carrier: 'Acme', premium: 100 }];
      const actual: Quote[] = [
        { carrier: 'Acme', premium: 100 },
  ...
  describe('objects with field comparators', () => {
    ...
    it('similarity score reflects field comparator results', async () => {
      const expected: Quote[] = [{ carrier: 'Acme', premium: 100 }];
      ...
      const strictResult = await matchArrays(expected, actual);
      ...
      const tolerantResult = await matchArrays(expected, actual, {
        premium: within({ tolerance: 0.1 }),
      });
      expect(tolerantResult.unmatchedExpected).toHaveLength(0);
    });

    it('matches reordered arrays using field comparators', async () => {
      const expected: Quote[] = [
        { carrier: 'Acme', premium: 100 },
        { carrier: 'Beta', premium: 200 },
      ];
    ...
    it('hungarian finds optimal pairing even with partial similarity', async () => {
      const expected: Quote[] = [{ carrier: 'Acme', premium: 100 }];
  ...
  describe('real-world quote matching scenarios', () => {
    it('hungarian algorithm pairs quotes by closest premium', async () => {
      const result = await matchArrays(testCase1Expected, testCase1Actual, {
    ...
    it('matches quotes even with exact matching (suboptimal similarity)', async () => {
      const result = await matchArrays(testCase1Expected, testCase1Actual);
    ...
    it('handles more actual items than expected items', async () => {
      const result = await matchArrays(testCase2Expected, testCase2Actual);
    ...
    it('matches closest date when actual > expected using date similarity', async () => {
      const result = await matchArrays(testCase2Expected, testCase2Actual, {
        effective_date: date,
      });
  ...
  describe('edge cases', () => {
    it('handles nested objects with similarity calculation', async () => {
    ...
    it('treats array fields as primitives requiring exact match', async () => {
    ...
    it('array fields with different values reduce similarity', async () => {
      const expected = [{ tags: ['a', 'b'], name: 'item1' }];
    ...
    it('handles undefined vs null field mismatch', async () => {
      const expected = [{ name: 'John', optional: null }];
    ...
    it('ignores extra fields in actual objects', async () => {
    ...
    it('handles all-identical items in both arrays', async () => {
    ...
    it('picks optimal match when competing similar objects exist', async () => {
    ...
    it('handles type mismatches between string and number', async () => {
    ...
    it('uses exact comparator for fields without explicit comparator', async () => {
    ...
    it('handles mixed comparators with some fields failing', async () => {
      const expected = [{ carrier: 'Acme', premium: 100, region: 'US' }];
      const actual = [
    ...
    it('handles objects with boolean fields', async () => {
  ...
  describe('optimal pairing verification', () => {
    it('pairs each expected to its exact match when available', async () => {
      const expected = [
        { carrier: 'Acme', premium: 100 },
        { carrier: 'Beta', premium: 200 },
    ...
    it('hungarian finds global optimum over greedy local choice', async () => {
    ...
    it('pairs by closest numeric value with within comparator', async () => {
      const expected = [
        { premium: 100 }, // E0
        { premium: 200 }, // E1
        { premium: 300 }, // E2
    ...
    it('pairs by closest date with date comparator', async () => {
      const expected = [
    ...
    it('optimizes multi-field matching for best overall pairing', async () => {
      const expected = [
    ...
    it('selects best actual when multiple compete for one expected', async () => {
    ...
    it('distributes actuals optimally when expected items compete', async () => {
    ...
    it('avoids suboptimal greedy pairing in complex scenario', async () => {
    ...
    it('handles asymmetric matching with more expected than actual', async () => {
      const expected = [
        { id: 1, score: 100 }, // E0
        { id: 2, score: 200 }, // E1
        { id: 3, score: 300 }, // E2
    ...
    it('handles asymmetric matching with more actual than expected', async () => {
      const expected = [
        { id: 2, score: 200 }, // E0
      ];
      const actual = [
  ...
  describe('nested array matching', () => {
    it('matches nested arrays of primitives in different order', async () => {
      const expected = [
        [1, 2],
        [3, 4],
      ];
    ...
    it('matches nested arrays of objects using field comparators', async () => {
      const expected = [[{ a: 1 }], [{ a: 2 }]];
      const actual = [[{ a: 2 }], [{ a: 1 }]];

      const result = await matchArrays(expected, actual, { a: exact });
    ...
    it('matches nested arrays with partial object matches', async () => {
    ...
    it('handles deeply nested arrays (3+ levels)', async () => {
      const expected = [[[1]], [[2]]];
      const actual = [[[2]], [[1]]];
    ...
    it('handles mixed nested structures', async () => {
      ...
      const result = await matchArrays(expected, actual, { id: exact });
    ...
    it('penalizes length mismatches in nested arrays', async () => {
    ...
    it('matches complex nested object arrays', async () => {
      const expected = [
        [
          { carrier: 'Acme', premium: 100 },
          { carrier: 'Beta', premium: 200 },
