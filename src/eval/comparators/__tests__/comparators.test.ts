...
describe('within', () => {
  it('percentage mode passes/fails based on tolerance', async () => {
    ...
    expect((await comparator(100, 103)).passed).toBe(true);
    expect((await comparator(100, 110)).passed).toBe(false);
  });

  it('absolute mode passes/fails based on tolerance', async () => {
    const comparator = within({ tolerance: 10, mode: 'absolute' });
    expect((await comparator(100, 108)).passed).toBe(true);
    expect((await comparator(100, 115)).passed).toBe(false);
  });

  it('percentage mode at exact boundary passes', async () => {
    const comparator = within({ tolerance: 0.05 });
    // 5% of 100 is exactly 5
    expect((await comparator(100, 105)).passed).toBe(true);
    expect((await comparator(100, 95)).passed).toBe(true);
  });

  it('percentage mode with expected=0 requires exact match', async () => {
    // When expected is 0, allowedDiff = 0 * tolerance = 0
    // Only exact match passes
    const comparator = within({ tolerance: 0.05 });
    expect((await comparator(0, 0)).passed).toBe(true);
    expect((await comparator(0, 0.001)).passed).toBe(false);
    expect((await comparator(0, -0.001)).passed).toBe(false);
  });

  it('absolute mode with negative numbers', async () => {
    const comparator = within({ tolerance: 10, mode: 'absolute' });
    expect((await comparator(-100, -95)).passed).toBe(true);
    expect((await comparator(-100, -115)).passed).toBe(false);
  });
...
describe('oneOf', () => {
  ...
  it('fails when value is not in allowed set', async () => {
    const result = await comparator('a', 'invalid' as any);
    expect(result.passed).toBe(false);
  });

  it('throws when allowedValues is empty', async () => {
...
describe('custom', () => {
  it('uses provided compare function', async () => {
    const comparator = custom({
      compare: (expected: number, actual: number) => actual > expected,
    });

    expect((await comparator(5, 10)).passed).toBe(true);
    expect((await comparator(5, 3)).passed).toBe(false);
  });
});
...
describe('exact', () => {
  ...
  it('deep equals arrays with objects', async () => {
    const arr = [{ id: 1 }, { id: 2 }];
...
describe('numeric', () => {
  ...
  it('handles accounting notation (parentheses for negative)', async () => {
  ...
  it('passes when both are null/empty', async () => {
  ...
  it('fails when only one is null/empty', async () => {
...
describe('date', () => {
  ...
  it('handles EU format when unambiguous (day > 12)', async () => {
  ...
  it('defaults to US format when ambiguous', async () => {
  ...
  it('fails for unparseable dates', async () => {
...
describe('name', () => {
  ...
  it('strips common business suffixes', async () => {
  ...
  it('matches when both have suffixes', async () => {
  ...
  it('handles middle names/initials by comparing first and last tokens', async () => {
  ...
  it('returns lower similarity for middle name matches vs exact matches', async () => {
  ...
  it('fails when first or last name differs despite middle name flexibility', async () => {
  ...
  it('requires exact match for single-token names', async () => {
  ...
  it('passes with fuzzy matching for slight typos', async () => {
  ...
  it('returns reduced similarity for fuzzy matches', async () => {
    const result = name('Johara A. Hassan', 'Johara A. Hasson');
    expect(result.similarity).toBeGreaterThanOrEqual(0.9);
    expect(result.similarity).toBeLessThan(0.95);
  });

  it('fails fuzzy matching when too many differences', async () => {
...
describe('similarity scores', () => {
  ...
  it('date returns 1.0 similarity for exact match', async () => {
    const result = date('2024-01-15', '2024-01-15');
    expect(result.similarity).toBe(1.0);
    expect(result.passed).toBe(true);
  });

  it('within returns higher similarity for closer values', async () => {
    const comparator = within({ tolerance: 0.1 });
    const result1 = await comparator(100, 102); // 2% diff
    const result2 = await comparator(100, 108); // 8% diff
  ...
  it('within returns 1.0 similarity for exact match', async () => {
    const comparator = within({ tolerance: 0.1 });
    const result = await comparator(100, 100);
    expect(result.similarity).toBe(1.0);
    expect(result.passed).toBe(true);
  });

  it('within returns ~0.5 similarity at tolerance boundary', async () => {
    const comparator = within({ tolerance: 0.1 });
    const result = await comparator(100, 110); // exactly 10% diff (at boundary)
...
describe('edge cases', () => {
  describe('within with special numeric values', () => {
    it('handles NaN values', async () => {
      ...
      expect((await comparator(NaN, 100)).passed).toBe(false);
      expect((await comparator(100, NaN)).passed).toBe(false);
      expect((await comparator(NaN, NaN)).passed).toBe(false);
    });

    it('handles Infinity values', async () => {
      const comparator = within({ tolerance: 0.1 });
      // Infinity - Infinity is NaN, NaN <= threshold is false
      expect((await comparator(Infinity, Infinity)).passed).toBe(false);
      ...
      expect((await comparator(Infinity, 100)).passed).toBe(true); // diff=Infinity, threshold=Infinity
      expect((await comparator(100, Infinity)).passed).toBe(false); // diff=Infinity, threshold=10
      expect((await comparator(-Infinity, -Infinity)).passed).toBe(false); // NaN
    });

    it('handles zero tolerance', async () => {
      const comparator = within({ tolerance: 0 });
      expect((await comparator(100, 100)).passed).toBe(true);
      expect((await comparator(100, 100.001)).passed).toBe(false);
    });
  });

  describe('numeric with special values', () => {
    it('returns null for NaN values (fails comparison)', async () => {
  ...
  describe('exact with circular references', () => {
    ...
    it('handles nested circular references', async () => {
      const obj1: any = { a: { b: {} } };
    ...
    it('handles array circular references', async () => {
      const arr1: any[] = [1, 2];
      arr1.push(arr1);
  ...
  describe('name with international characters', () => {
    it('computes similarity for accented characters', async () => {
    ...
    it('handles names with umlauts via fuzzy matching', async () => {
  ...
  describe('contains with special characters', () => {
    it('handles regex special characters literally', async () => {
      ...
      expect((await comparator('', 'result [test] string')).passed).toBe(true);
      expect((await comparator('', 'result test string')).passed).toBe(false);
    });

    it('handles empty string substring', async () => {
